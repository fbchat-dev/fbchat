...
from fbchat import Client, Group

# Change this to your group id
my_thread = Group(1234567890)


class KeepBot(Client):
    def on_image_set(self, thread, actor, old_image):
        if my_thread == thread and self.user != actor:
    ...
    def on_user_added(self, thread, actor, subject):
        if my_thread == thread and self.user not in [actor, subject]:
    ...
    def on_admin_added(self, thread, actor, subject):
        if my_thread == thread and self.user not in [actor, subject]:
            print("{.name} got added as admin".format(subject))
            self.remove_admin(thread, subject)

    def on_admin_removed(self, thread, actor, subject):
        if my_thread == thread and self.user not in [actor, subject]:
            print("{.name} got removed as admin".format(subject))
            self.add_admin(thread, subject)


bot = KeepBot.login("<email>", "<password>")
bot.listen()
